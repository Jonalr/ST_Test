                                    ...
                                    if(preg_match_all("/\[[a-zA-Z\d]{12}\]/", $temail->Subject . $temail->Body . $temail->BodyHTML, $matches))
                                    {
                                        ...
                                        foreach($matches[0] as $match)
                                        {
                                            ...
                                            if($exists=Ticket::Exists($match,$id,$groupid,$language))
                                            {
                                                ...
                                                $Ticket->Reactivate();
                                                $Ticket->SetLastUpdate(time(),$lastOutgoing);
                                                break;
                                            }
                                        }
                                    }
...
class Post extends BaseObject
{
    ...
    static function ProcessPostForExternal($_operator,$_postId,$rec,$recgroup,$post,$time,$_group,$_translation,$_translationISO,$_whisper=false)
    {
        ...
        $npost = new Post($_postId,$_operator->SystemId,"",$post,$time,$chatId,$_operator->Fullname);
    ...
    static function CloseChats()
    {
        ...
        while($row = DBManager::FetchArray($result))
        {
            ...
            if($results && $rows = DBManager::FetchArray($results))
            {
                ...
                if(!empty(Server::$Configuration->File["gl_actt"]) && $rows["internal_active"] == 0 && !$botchat)
                {
...
class Ticket extends Action
{
	...
	function GetXML($_full,$_demand=false)
	{
		if($_full)
		{
			...
			$xml = "<val id=\"".base64_encode($this->Id)."\" u=\"".base64_encode($this->LastUpdated)."\" w=\"".base64_encode($wb)."\" sa=\"".base64_encode($this->Salt)."\" del=\"".base64_encode($this->Deleted ? "1" : "0")."\" wl=\"".base64_encode($this->WatchList ? "1" : "0")."\" gr=\"".base64_encode($this->Group)."\" uid=\"".base64_encode($this->SenderUserId)."\" l=\"".base64_encode($this->Language)."\" h=\"".base64_encode($this->GetHash())."\" p=\"".base64_encode($this->Priority)."\" t=\"".base64_encode($this->Channel)."\" s=\"".base64_encode($this->SubChannel)."\">\r\n";
    ...
    function SendOperatorReply($_messageId, $_email, $_qMessageId = "", $_ecc = "", $_ebcc = "")
    {
        ...
        for($i=0;$i<count($tsData);$i++)
        {
            ...
            //$tsData[$i] = $this->TextReplace($tsData[$i]); 7.0.9.3 MF
            ...
            $tsData[$i] = $this->TextReplace($tsData[$i]); //7.0.9.3 MT
        }
    ...
    static function CreateFromChatArchive($_chatId)
    {
        ...
        $ticket = Ticket::GetOpenByUserId($achat->VisitorId);
...
class EventAction
{
    ...
    function Execute($_param,$_visitor,$_chat=null)
    {
        if($this->Type == 7)
        {
            ...
            $context  = stream_context_create($opts);
            $response = file_get_contents($object->Url, false, $context);
...
class Event extends BaseObject
{
	...
	function MatchVisitorConditions($_pageCount,$_timeOnSite,$_invAccepted,$_invDeclined,$_inChat,$_searchPhrase="",$_isMobile=false,$_country="")
	{
        ...
        $_result["iic"] = (!empty($this->TriggerChatCreated) && !$_inChat);
		...
		else if($this->TimeOnSite > 0 && $_timeOnSite < ($this->TimeOnSite-3))
        {
			...
			$match = false;
        else if(!empty($this->TriggerChatCreated) && !$_inChat)
            $match = false;
...
class ChatArchive
{
    static function GetSearchQuery($_sFor,$_user=false)
    {
        ...
        return "LOWER(`fullname`) LIKE '%".$_sFor."%' OR LOWER(`area_code`) LIKE '%".$_sFor."%' OR LOWER(`plaintext`) LIKE '%".$_sFor."%' OR LOWER(`email`) LIKE '%".$_sFor."%' OR LOWER(`company`) LIKE '%".$_sFor."%' OR LOWER(`phone`) LIKE '%".$_sFor."%' OR LOWER(`chat_id`) LIKE '%".$_sFor."%' OR LOWER(`external_id`) LIKE '%".$_sFor."%' OR LOWER(`customs`) LIKE '%".$_sFor."%' OR LOWER(`question`) LIKE '%".$_sFor."%'";
    }
...
class KnowledgeBase
{
    ...
    static function CreateEntry($_userId, $_resId, $_value, $_type, $_title, $_disc, $_parentId, $_size=0, $_tags = "")
    {
        if($_size == 0)
            $_size = strlen($_title);
...
class KnowledgeBaseEntry
{
    ...
    function ValidateParents($_root="")
    {
        if(empty($_root) || $this->Id == $_root)
            return true;
        ...
        while(!$stop)
        {
            ...
            if($result = DBManager::Execute(true, "SELECT * FROM `" . DB_PREFIX . DATABASE_RESOURCES . "` WHERE `id` = '" . DBManager::RealEscape($pid) . "' AND `type`=0 AND `discarded`=0"))
            {
                if($row = DBManager::FetchArray($result))
                {
                    if($row["id"] == $_root)
                ...
                else
                {
                    ...
                    // correct it
                    Logging::DebugLog("INVALID ENTRY PARENT FOUND: " . $this->Id);
                    //DBManager::Execute(true, "UPDATE `" . DB_PREFIX . DATABASE_RESOURCES . "` SET `parentid`=1,edited=".time()." WHERE `id` = '" . DBManager::RealEscape($this->Id) . "' LIMIT 1;");
                    return false;
                }
            }
        }
...
class Cookie
{
    static function Set($_key,$_value,$_onlyWhenEmpty=false)
    {
        if(!empty(Server::$Configuration->File["gl_colt"]) && !empty($_value))
        {
            ...
            setcookie("lz_" . $_key,($_COOKIE["lz_" . $_key] = base64_encode($_value)),$lifetime,null,null,null,true);
            setcookie("livezilla", "", time()-3600,null,null,null,true);
        }
    }
...
class GeoTracking
{
    ...
    static function Replace($_toReplace, $jsa = "")
    {
        ...
        {
            $gkey = isset(Server::$Configuration->File["gl_pr_ngl"]) && !empty(Server::$Configuration->File["gl_pr_ngl"]) ? Server::$Configuration->File["gl_pr_ngl"] : "FU";
