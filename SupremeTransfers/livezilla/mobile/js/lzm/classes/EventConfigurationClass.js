...
EventConfiguration.prototype.showEventSubElementCreation = function(action,type,id){
    ...
    dHtml += '<fieldset id="ev-sub-element-settings" class="lzm-fieldset-full top-space-double" style="width:450px;min-height:390px;"><legend>'+tid('settings')+'</legend><div id="element-configuration"></div></fieldset>';
    ...
    elementSelect.change(function(){
        ...
        else
        {
            ...
            if(d(def.operatorList)){
                ...
                html += '<label class="top-space-double">'+tidc('operators')+'</label><div class="border-s" style="height:240px;overflow:auto;">' + lzm_inputControls.CreateOperatorList(def.name+'-receivers',false,false,recList) + '</div>';
            }
...
EventConfiguration.prototype.applySubElementCustomLogic = function(definition,obj,objId){
    ...
    else if(definition.name == 'ExternalInvitation')
    {
        ...
        ithtml += '<div class="left-space-child">' + lzm_inputControls.createArea(prefix + 'a-ict',cText,'','','height:200px;') + '<br><br></div>';
...
EventConfiguration.prototype.createEventHtml = function(tab){
    ...
    if(tab==0)
    {
        ...
        thtml += '<table class="top-space"><tr><td style="padding-top:18px;width:90px;">' + lzm_inputControls.createCheckbox('cb-event-active',tid('active'),(this.m_SelectedEvent.ia=='1'),'') + '</td><td>' + lzm_inputControls.createInput('s-event-name', '', this.m_SelectedEvent.name, tidc('name'), '', 'text', '') +'</td>';
        thtml += '<td>' + lzm_inputControls.createPriorityList('event-priority','',this.m_SelectedEvent.prio,100,tidc('priority')) +'</td></tr></table>';
        thtml += '<table class="top-space">';
        thtml += '<tr><td style="width:50%;vertical-align: top;">'+this.createConditionsHtml()+'</td><td></td><td style="width:49%;vertical-align: top;">'+this.createActionsHtml()+'</td></tr>';
        thtml += '</table></div>';
    }
    else if(tab==1)
    {
...
EventConfiguration.prototype.createEventSubRowHtml = function(obj,name,title,type,id,edit) {
    ...
    if(edit)
        rowHtml += lzm_inputControls.createButton('edit-' + fid, '', 'showEventSubElementCreation(\'edit\',\''+type+'\',\''+fid+'\');', tid('edit'), '<i class="fa fa-gear icon-small"></i>', 'lr',{'margin-left': '4px'},'',30,'e');
    rowHtml += lzm_inputControls.createButton('remove-' + fid, '', 'showEventSubElementCreation(\'remove\',\''+type+'\',\''+fid+'\');', tid('delete'), '<i class="fa fa-trash icon-small"></i>', 'lr',{'margin-left': '4px'},'',30,'e')+'</td></tr>';
    return rowHtml;
};
