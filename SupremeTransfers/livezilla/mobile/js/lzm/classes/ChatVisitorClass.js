...
ChatVisitorClass.prototype.CreateVisitorListLine = function(_visitorObj, newLine, updateLine, _fullscreen){
    ...
    if (visitorIsChatting)
        extUserHtmlString += '<td class="icon-column nobg noibg"><i class="fa fa-comments icon-orange"></i></td>';
    else
...
ChatVisitorClass.prototype.CreateVisitorInformationTable = function(_visitorObj, elementId) {
    ...
    if (d(_visitorObj.lang))
    {
        ...
        var chatObj = DataEngine.ChatManager.GetChat(_visitorObj.i,'v');
/*
...
ChatVisitorClass.prototype.EditVisitorDetails = function(visitorId,field,elementId){
    ...
    inputForm = '<fieldset class="lzm-fieldset-full">'+inputForm+'</fieldset>';
...
ChatVisitorClass.prototype.createVisitorInvitation = function(visitor) {
    ...
    // group
    var groupSelectHtml = '<label for="group-selection">' + tidc('group') + '</label><select id="group-selection">';
    ...
    var browserSelectHtml = '<label for="browser-selection" class="top-space">' + tidc('browser') + '</label><select id="browser-selection" class="lzm-multiselect" size="3" style="height:86px;">';
...
ChatVisitorClass.prototype.createCustomInputString = function(visitor, inputId, _makeClickable) {
    var val = DataEngine.inputList.getInputValueFromVisitor(inputId,visitor).replace(/-/g,'&#8209;').replace(/ /g,'&nbsp;');
    if(_makeClickable && val != null && val != '' && val.toLowerCase().indexOf('http') == 0)
        val = '<a href="#" class="lz_chat_link_no_icon" onclick="openLink(\'http://dereferrer.livezilla.info/?turl=' + encodeURIComponent(val) + '\');">' + val + '</a>';
    return val;
};
...
ChatVisitorClass.prototype.RenderVisitorLayout = function(){
    ...
    var avWidth = (UIRenderer.windowWidth - ((CommonUIClass.BlockNavigationHidden) ? 0 : 70))-18;
    if(LocalConfiguration.VisitorsMapVisible)
    {
